 ...
 * License: MIT
 *
 * Edited to use https://github.com/cmackay/google-analytics-plugin
 */
(function(angular) {
  'use strict';
   ...
   */
  angular.module('angulartics.google.analytics.cordova.cmackay', ['angulartics'])
        ...
        function ($q, $log, ready, debug, trackingId, period) {
          ...
          this.init = function () {
            return deferred.promise.then(function () {
              var analytics = window.plugins && window.navigator.analytics;
              if (analytics) {
                analytics.setTrackingId(trackingId, function onInit() {
      ...
      return {
        $get: ['$injector', function ($injector) {
          return $injector.instantiate(GoogleAnalyticsCordova, {
            ...
            period: this.period // not supported
          });
        }],
        ready: function (fn) {
          this._ready = fn;
    ...
    .config(['$analyticsProvider', 'googleAnalyticsCordovaProvider', function ($analyticsProvider, googleAnalyticsCordovaProvider) {
      googleAnalyticsCordovaProvider.ready(function (analytics, success, failure) {
        $analyticsProvider.registerPageTrack(function (path) {
          analytics.sendAppView(path, success, failure);
        });

        $analyticsProvider.registerEventTrack(function (action, properties) {
          analytics.sendEvent(properties.category, action, properties.label, properties.value, success, failure);
        });
      });
    }])
